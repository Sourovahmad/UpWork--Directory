...
// chart 1 ..
$chart_1 = array();
$chart_1[] = count(DB::table("rc_users")->where("type", 1)->where("date_insert", ">=", "$year_before-1-1")->where("date_insert", "<=", "$year_before-12-31")->get());

// chart 2 ..
$chart_2 = array();
$chart_2[] = count(DB::table("rc_users")->where("type", 2)->where("date_insert", ">=", "$year_before-1-1")->where("date_insert", "<=", "$year_before-12-31")->get());

// chart 3 ..
$chart_3 = array();
$chart_3[] = count(DB::table("tb_hospital")->where("date_insert", ">=", "$year_before-1-1")->where("date_insert", "<=", "$year_before-12-31")->get());

// chart 4 ..
$chart_4 = array();
$chart_4[] = count(DB::table("tb_contact")->where("date_insert", ">=", "$year_before-1-1")->where("date_insert", "<=", "$year_before-12-31")->get());
...
for ($x = 1; $x <= date("m"); $x++):
    ...
    $chart_1[] = count(DB::table("rc_users")->where("type", 1)->where("date_insert", ">=", "$year-$x-1")->where("date_insert", "<=", "$year-$x-31")->get());
    $chart_2[] = count(DB::table("rc_users")->where("type", 2)->where("date_insert", ">=", "$year-$x-1")->where("date_insert", "<=", "$year-$x-31")->get());
    $chart_3[] = count(DB::table("tb_hospital")->where("date_insert", ">=", "$year-$x-1")->where("date_insert", "<=", "$year-$x-31")->get());
    $chart_4[] = count(DB::table("tb_contact")->where("date_insert", ">=", "$year-$x-1")->where("date_insert", "<=", "$year-$x-31")->get());
endfor;
?>
...
<div class="row">
            ...
            <div class="card-body card-body pb-0 d-flex justify-content-between align-items-start">
                <div>
                    <div class="text-value-lg">
                        <?= count(DB::table("rc_users")->where("type", 1)->get()); ?>
                    </div>
                    <div>العملاء </div>
                </div>
                <div class="btn-group">
                    ...
                    <div class="dropdown-menu dropdown-menu-right">
                        <a class="dropdown-item" href="<?= RC_url("users") ?>">عرض الأعضاء</a>
                    </div>
                </div>
            </div>
            <div class="c-chart-wrapper mt-3 mx-3" style="height:70px;">
                <canvas class="chart" key="<?= implode(",", $mothes) ?>" label=" عضو " val="<?= implode(",", $chart_1) ?>" id="card-chart1" height="70"></canvas>
            </div>
        </div>
    </div>
    ...
    <div class="col-sm-6 col-lg-3">
        <div class="card text-white bg-gradient-info">
            <div class="card-body card-body pb-0 d-flex justify-content-between align-items-start">
                <div>
                    <div class="text-value-lg"><?= count(DB::table("rc_users")->where("type", 2)->get()); ?></div>
                    <div>مقدمي الخدمة</div>
                </div>
                <div class="btn-group">
                    <button class="btn btn-transparent dropdown-toggle p-0" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                        <svg class="c-icon">
            ...
            <div class="c-chart-wrapper mt-3 mx-3" style="height:70px;">
                <canvas class="chart" key="<?= implode(",", $mothes) ?>" label=" عضو " val="<?= implode(",", $chart_2) ?>" id="card-chart2" height="70"></canvas>
            </div>
        </div>
    </div>
    ...
    <div class="col-sm-6 col-lg-3">
        <div class="card text-white bg-gradient-warning">
            <div class="card-body card-body pb-0 d-flex justify-content-between align-items-start">
                <div>
                    <div class="text-value-lg"><?= count(DB::table("tb_hospital")->get()); ?></div>
                    <div>العيادات</div>
                </div>
                <div class="btn-group">
                    <button class="btn btn-transparent dropdown-toggle p-0" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                    ...
                    <div class="dropdown-menu dropdown-menu-right">
                        <a class="dropdown-item" href="<?= RC_url("hospital") ?>">عرض العيادات</a>
                    </div>
                </div>
            </div>
            <div class="c-chart-wrapper mt-3" style="height:70px;">
                <canvas class="chart"  key="<?= implode(",", $mothes) ?>" label=" عيادة " val="<?= implode(",", $chart_3) ?>" id="card-chart3" height="70"></canvas>
            </div>
        </div>
    </div>
    ...
    <div class="col-sm-6 col-lg-3">
        <div class="card text-white bg-gradient-danger">
            <div class="card-body card-body pb-0 d-flex justify-content-between align-items-start">
                <div>
                    <div class="text-value-lg"><?= count(DB::table("tb_contact")->get()); ?></div>
                ...
                <div class="btn-group">
                    ...
                    <div class="dropdown-menu dropdown-menu-right">
                        <a class="dropdown-item" href="<?= RC_url("contact") ?>">عرض رسائل التواصل</a>
                    </div>
                </div>
            </div>
            <div class="c-chart-wrapper mt-3 mx-3" style="height:70px;">
                <canvas class="chart" key="<?= implode(",", $mothes) ?>" label=" رسالة " val="<?= implode(",", $chart_4) ?>" id="card-chart4" height="70"></canvas>
            </div>
        </div>
    </div>
            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-sm-12">
                        <div class="row">
                            <div class="col-4">
                                <div class="c-callout c-callout-info"><small class="text-muted">تحت المراجعه</small>
                                    <div class="text-value-lg">
                                        <?= count(DB::table("tb_commission")->where("status", null)->get()) ?>
                                    </div>
                                </div>
                            </div>

                            <div class="col-4">
                                <div class="c-callout c-callout-success"><small class="text-muted">مقبولة</small>
                                    <div class="text-value-lg">          <?= count(DB::table("tb_commission")->where("status", 1)->get()) ?></div>
                                </div>
                            </div>

                            <div class="col-4">
                                <div class="c-callout c-callout-danger"><small class="text-muted">مرفوضة</small>
                                    <div class="text-value-lg">          <?= count(DB::table("tb_commission")->where("status", 2)->get()) ?></div>
                                </div>
                            </div>
                ...
                <table class="table table-responsive-sm table-hover table-outline mb-0">
                    <thead class="thead-light">
                        <tr>
                            <th>العضو</th>
                            <th class="text-center">إسم المحول	</th>
                            <th>المبلغ</th>
                        ...
                        <?php foreach ($q as $r): ?>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <div class="small text-muted"><span><?= @$user->mobile ?></span> | <?= @$user->email ?></div>
                                </td>
    ...
    <div class="card">
        <div class="card-body">
            <div class="d-flex justify-content-between">
                <div>
                    <h4 class="card-title mb-0">المبيعات</h4>
...
<div class="row">
    <div class="col-sm-6 col-lg-4">
        <div class="card">
            <div class="card-header bg-facebook content-center">
                <h3 class="fs-1 text-white my-4 pt-2 pb-2">
                    الأعضاء الجدد
                </h3>
            </div>
            <div class="card-body row text-center">
                <div class="col">
                    <div class="text-value-xl"><?= count(DB::table("rc_users")->where("type", 1)->where("view", 0)->get()); ?></div>
                    <div class="text-uppercase text-muted small">عميل</div>
                </div>
                <div class="c-vr"></div>
                <div class="col">
                    <div class="text-value-xl"><?= count(DB::table("rc_users")->where("type", 2)->where("view", 0)->get()); ?></div>
                    <div class="text-uppercase text-muted small">مقدم خدمة</div>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="col-sm-6 col-lg-4">
        <div class="card">
            ...
            <div class="card-body row text-center">
                <div class="col">
                    <div class="text-value-xl"><?= count(DB::table("tb_hospital")->where("view", 0)->get()); ?></div>
                    <div class="text-uppercase text-muted small">جديدة</div>
                </div>
                <div class="c-vr"></div>
                <div class="col">
                    <div class="text-value-xl"><?= count(DB::table("tb_hospital")->get()); ?></div>
                    <div class="text-uppercase text-muted small">الكل</div>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="col-sm-6 col-lg-4">
        <div class="card">
            ...
            <div class="card-body row text-center">
                <div class="col">
                    <div class="text-value-xl"><?= count(DB::table("tb_contact")->where("view", 0)->get()); ?></div>
                    <div class="text-uppercase text-muted small">جديدة</div>
                </div>
                <div class="c-vr"></div>
                <div class="col">
                    <div class="text-value-xl"><?= count(DB::table("tb_contact")->get()); ?></div>
                    <div class="text-uppercase text-muted small">الكل</div>
                </div>
            </div>
        </div>
